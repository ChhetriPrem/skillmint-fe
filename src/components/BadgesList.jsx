...
export default function BadgeList() {
  ...
  if (!publicKey) return <div className="text-center text-gray-400 mt-8">Connect your wallet to see badges.</div>;
  if (loading) return <div className="text-center text-purple-400 mt-8 animate-pulse">Loading badges...</div>;
  if (badges.length === 0) return <div className="text-center text-gray-400 mt-8">No badges found.</div>;
      ...
      {badges.map(badge => (
        ...
        >
          <img
            ...
            onError={e => { e.target.src = "https://placehold.co/96x96?text=NFT"; }}
          />
          <h3 className="text-2xl font-bold text-purple-200 mb-1">{badge.badge_name}</h3>
          ...
          <div className="flex flex-col gap-1 text-sm text-gray-300 w-full">
            <div><b>Template ID:</b> <span className="text-white">{badge.template_id}</span></div>
            <div><b>Receiver:</b> <span className="text-white">{badge.receiver_github}</span></div>
            <div><b>Reviewer:</b> <span className="text-white">{badge.reviewer}</span></div>
            <div>
              <b>PR Link:</b>{" "}
              <a
                href={badge.pr_link}
